...
public class CheckDeclarations extends ASTNodeVisitor<Boolean> {
    ...
    public Boolean visit(ArrayLocationNode node) {
        ...
        if (search == null)
            ErrorReporting.reportError(new SymbolTableException(node.getSourceLoc(), "Undeclared variable"));
    ...
    public Boolean visit(MethodCallNode node) {
        if (!methodTable.contains(node.getName()))
            ErrorReporting.reportError(new SymbolTableException(node.getSourceLoc(), "Undeclared method"));
